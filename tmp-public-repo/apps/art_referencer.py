...
from pytools import Logger, LogLevel, TermArtist as ta
...
def main():
    ...
    if not SAVE_PATH.exists():
        LOGGER.info("Creating the save path: ", SAVE_PATH, start=ta.FG.GREEN)
    ...
    def scroll_to_bottom(num):
        for _ in range(num): nw.scroll_to_window_scrollHeight(); nw.sleep(1);
        

    def search_artstation():
        for keyword in KEYWORDS:
            nw.bot.get("https://www.artstation.com/search")
            search_bar = nw.get_elements_by_xpath(
            ...
            img_blocks = nw.get_elements_by_xpath(
            ...
            def fetch_images_from_page(page_i, page, save_path, keyword):
                nw.sleep(1.5)
                imgs = nw.get_elements_by_xpath("//img[contains(@class, 'img img-fluid')]")
